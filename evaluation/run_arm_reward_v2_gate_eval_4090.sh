...
# This freezes the step-6050 anchor and candidate into immutable bundles, then
...
V2_ROOT="${V2_ROOT:-/sda/mashijian/doorman_arm_reward_v2_20260822}"
RUN_DIR="${RUN_DIR:-$V2_ROOT/runs/door_open_homie_arm_reward_v2}"
CODE_DIR="${CODE_DIR:-$V2_ROOT/code}"
ANCHOR="${ANCHOR:-$RUN_DIR/checkpoint_anchor/model_step_006050.pt}"
CANDIDATE_STEP="${CANDIDATE_STEP:-6100}"
...
COMPARATOR="${COMPARATOR:-$V2_ROOT/compare_arm_reward_v2_gate.py}"
...
mkdir -p "$OUTPUT_ROOT/checkpoints/step6050" "$OUTPUT_ROOT/checkpoints/$CANDIDATE_LABEL"
cp --reflink=auto "$ANCHOR" "$OUTPUT_ROOT/checkpoints/step6050/model_step_006050.pt"
cp "$RUN_DIR/config.yaml" "$OUTPUT_ROOT/checkpoints/step6050/config.yaml"
...
sha256sum \
    "$OUTPUT_ROOT/checkpoints/step6050/model_step_006050.pt" \
    "$OUTPUT_ROOT/checkpoints/step6050/config.yaml" \
...
python - \
    "$OUTPUT_ROOT/checkpoints/step6050/model_step_006050.pt" 6050 \
...
(
    ...
    CHECKPOINT="$OUTPUT_ROOT/checkpoints/step6050/model_step_006050.pt" \
    OUTPUT_DIR="$OUTPUT_ROOT/evals/step6050" \
    GPU="$ANCHOR_GPU" \
    NUM_ENVS="$NUM_ENVS" \
    NUM_EPISODES="$NUM_EPISODES" \
    EVAL_NAME="arm_v2_gate_step6050" \
    bash "$INSTRUMENTATION_DIR/run_diagnostics_v2.sh"
) >"$OUTPUT_ROOT/evals/step6050.supervisor.log" 2>&1 &
anchor_pid=$!
...
(
    ...
    NUM_ENVS="$NUM_ENVS" \
    NUM_EPISODES="$NUM_EPISODES" \
    EVAL_NAME="arm_v2_gate_$CANDIDATE_LABEL" \
    bash "$INSTRUMENTATION_DIR/run_diagnostics_v2.sh"
...
if (( anchor_status != 0 || candidate_status != 0 )); then
    printf 'Paired diagnostic failed: step6050=%d %s=%d\n' \
        "$anchor_status" "$CANDIDATE_LABEL" "$candidate_status" >&2
    exit 1
fi

for step in step6050 "$CANDIDATE_LABEL"; do
...
python "$COMPARATOR" \
    "$OUTPUT_ROOT/evals/step6050/diag/post_open_analysis_v2.json" \
...
printf 'Paired arm-v2 gate diagnostic complete: %s\n' "$OUTPUT_ROOT"
